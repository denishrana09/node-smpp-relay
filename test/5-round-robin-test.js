...
async function testRoundRobin() {
  ...
  try {
    ...
    for (let batch = 0; batch < 3; batch++) {
      console.log(`\nBatch ${batch + 1}:`);
      for (let i = 0; i < 3; i++) {
        await client3.sendMessage(`RR-Test-B${batch + 1}-${i + 1}`, '1234567890');
        await new Promise(resolve => setTimeout(resolve, 200));
      }
    }

    console.log('\nPhase 2: Testing with vendor downtime...');
    console.log('Please stop vendor2 (port 2777) now and press Enter to continue...');
    ...
    const phase1Messages = messages.filter(m => m.content.includes('RR-Test-B'));
    console.log('\nPhase 1 (Initial round-robin):');
    analyzeDistribution(phase1Messages);

    // Analyze distribution during vendor downtime
    const phase2Messages = messages.filter(m => m.content.includes('RR-Test-Down'));
    console.log('\nPhase 2 (One vendor down):');
    analyzeDistribution(phase2Messages);

    // Analyze distribution after vendor restoration
    const phase3Messages = messages.filter(m => m.content.includes('RR-Test-Final'));
